    ...
    if t == "BGPLS_STATE":
      ...
      if ev_state == "SYNCING":
        ...
        if self.bgpls_active == False:
          ...
          for const in constinfo.keys():
            ...
            #G_Gc[const]  = G_GM.make_G(G_G_base, const, G_CONSTINFO[const])
      ...
      if ( ev["diff"]["type"] == DiffType.DEL ): # Just Del
        ...
        if self.bgpls_active == True:
          ...
          #G_Gc[cid]   = self.make_G(G_G_base, cid, ev["diff"]["new"])
          #G_Gc_t[cid] = ev_time
      ...
      #self.snap_G_base    = copy.deepcopy(G_G_base)
      #self.snap_G_base_t  = G_G_base_t
      self.snap_G_base    = copy.deepcopy(self.G_base)
      self.snap_G_base_t  = self.G_base_t
      #self.snap_G_base    = copy.deepcopy(G_G_base)
      #self.snap_G_base_t  = G_G_base_t
      #self.snap_Gc        = copy.deepcopy(G_Gc)
      self.snap_Gc        = copy.deepcopy(self.Gc)
      #self.snap_Gc_t      = copy.deepcopy(G_Gc_t)
      self.snap_Gc_t      = copy.deepcopy(self.Gc_t)
      #self.snap_changed   = copy.deepcopy(self.changed)
      ...
      if "igp_metric" in lsattr:
        ...
        if key in self.linkstate:
          ...
          if ( self.bgpls_active == True ):
            ...
            self.linkstate[key]["state"]  = "up"
            self.linkstate[key]["statetime"] = ev_time
            self.linkstate[key]["cstate"] = "up" # INIT
  ...
  def check_link_state(self):
    ...
    while True:
      ...
      for k in self.linkstate.keys():
        ...
        elif self.linkstate[k]["cstate"] == "down":
...
def compute_paths5(comptype):
  ...
  wk_G_base        = copy.deepcopy(G_GM.snap_G_base)
  wk_G_base_t      = copy.deepcopy(G_GM.snap_G_base_t)
  wk_graphs        = copy.deepcopy(G_GM.snap_Gc)
  wk_graphs_t      = copy.deepcopy(G_GM.snap_Gc_t)

  wk_link_state    = copy.deepcopy(G_GM.linkstate)
  #print("wk_link_state")
  #print(wk_link_state)
  ...
  for pd in normalized:
    ...
    if ( comptype == "NWCHANGE" ):
      if pd.name in G_PATH.keys():
        ...
        if G_PATH[pd.name]["time"] == wk_graphs[pd.underlay]:
...
def compute_paths6(pd, wkG, rwkG, wk_G_base_t, wk_Gc_t):
  ...
  #G_LOG.info("[COMPUTE] path name:" + str(pd.name))
  G_LOG.info("[COMPUTE] path def:" + str(pd))
