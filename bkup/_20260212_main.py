...
from utils.loadclass import PathDef
from utils.load    import ConfigManager
import logging
import queue
  ...
  def handle_link(self, ev, ev_time):
    ...
    if None not in key:
      ...
      if "igp_metric" in lsattr:
        ...
        # bw
        self.BM.updbw(key,self.linkstate[key]["unrsvbw"])
...
class BWManager:
  ...
  def chkbw(self, lkey, bw):
    if (self.BWdb[lkey]["unrsvbw"] - self.BWdb[lkey]["sumpbw"] - bw ) >= 0:
      return True
    else:
      return False
...
def normalize_pathdef(name: str, raw: dict) -> PathDef:
...
def handle_event(ev):
  ...
  global G_PATH
  G_LOG.info("[COMPUTE] HANDLE EVENT")
...
def main():
  ...
  global G_CONSTINFO, G_NODEINFO, G_C_Queue
